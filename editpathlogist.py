...
"")
    ...
    def populate_refdr_data(self):
        if self.DoctorCode:
            ...
            if refdr_data:
                ...
                self.lineEdit_16.setText(str(refdr_data[0]))
                self.lineEdit_15.setText(refdr_data[1])  # Assuming the second item is the title
                self.lineEdit_18.setText(refdr_data[2])  # Assuming the third item is the patient name
                self.lineEdit_6.setText(refdr_data[3])
                self.lineEdit_12.setText(refdr_data[4])
                self.lineEdit_10.setText(refdr_data[5])
                self.lineEdit_14.setText(refdr_data[6])
                self.lineEdit_21.setText(refdr_data[7])
    ...
    def save_refdr_data(self):
        ...
        self.cursor.execute("UPDATE pathologist SET  DoctorName=?, Qualification=?, Specialisation=?, Address=?, PINCode=?, Mobile=?, Emailid=? WHERE DoctorCode=?", 
                            ( DoctorName, Qualification, Specialisation, Address, PINCode, Mobile,Emailid,DoctorCode))
        self.conn.commit()
